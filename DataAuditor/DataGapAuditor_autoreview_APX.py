...
# Creating a for loop to assign dummy variables to the Data Gap Auditor report:
        for n in range(0, excel_file.shape[1]):
            ...
            for m,p in enumerate(excel_file[excel_file.columns[(n)]]):
                ...
                try:
                    ...
                    if "<NO APX> / " in p:
                        excel_file[excel_file.columns[n]][m] = excel_file[excel_file.columns[n]][m].replace(p, '') # "Data not in the Vault"
                    elif " / <NO DATA>" in p:
                        excel_file[excel_file.columns[n]][m] = excel_file[excel_file.columns[n]][m].replace(p, '1') # "Data not in the database" -> APX Priority to put the data in the DBs
                    elif " / " in p:
                        excel_file[excel_file.columns[n]][m] = excel_file[excel_file.columns[n]][m].replace(p, '') # "Data not matching"  
                    else:
        ...
        for m,p in enumerate(excel_file['Review']):
            ...
            if (any('1' in k for k in p) and any('0' in k for k in p)):
                excel_file['Review'][m] = excel_file['Review'][m].replace(p, 'Data not in the database')
        ...
        for m,p in enumerate(zip(excel_file['Review'],excel_file.index)):
            ...
            if p[0] == 'Data not in the database':
        ...
        if periods_1 := periods_1: description.append("● Data not in the database for the periods: {}\n".format((list(set(periods_1)))).replace("'",'').replace('[','').replace(']',''))
        #0 "Complete"
    ...
    dict = {'Database': excel_file_content.iloc[4][1],      # Database name e.g. "Wilshire"
            excel_file_orig.iloc[6][1]: description,        # Product/vehicle name with review description of findings e.g. "Core Fixed Income Composite (P73285)"
            }  
